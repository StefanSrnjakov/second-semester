...
FileHandler::FileHandler(ExistingFileTag, 
    ...
    size_t expected_size = total_points * sizeof(Point);
    
    if (file_size != expected_size) {
        std::cout << "Warning: Binary file size (" << file_size 
                  << " bytes) does not match expected size for " 
                  << total_points << " points (" << expected_size << " bytes)" << std::endl;
    }
